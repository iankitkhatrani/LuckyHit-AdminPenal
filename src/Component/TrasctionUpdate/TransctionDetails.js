...
import styles from './TransctionDetails.module.css';
...
const TransctionDetails = () => {
    ...
    const navigateToContacts = () => {
        ...
        navigate('/depositmanagement');
    };

    const context = useContext(offerContext)
    const { DepositeData,DepositeUpdate,UploadScreenshort,host } = context
    ...
    const handleImage = async (e) =>{
        ...
        const value = await UploadScreenshort(e.target.files[0])
    ...
    const handleSubmit = async (event) => {
        ...
        let res = await DepositeUpdate(userInfo)
