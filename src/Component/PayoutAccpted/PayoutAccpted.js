...
import styles from './PayoutAccpted.module.css';
...
const PayoutAccpted = () => {
  ...
  const { PayoutAccptedList,DepositeDelete,host } = context
  ...
  const navigateToContacts = (trnsId,approve,reject,status,screenshort) => {
    ...
    navigate('/payouttrasctionupdate', {state:{trnsId,approve,reject,status,screenshort}});
  };
